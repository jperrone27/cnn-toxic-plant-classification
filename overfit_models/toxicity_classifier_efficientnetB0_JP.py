...
from keras.applications import ResNet50, MobileNet, EfficientNetB0
...
def run_model_efficientnet(X, Y, x_test, y_test, img_res, num_epochs, optim, validation = None, batch =None, log_file_no='', color = 'rgb', unfrozen = 2):
   ...
   base_model = EfficientNetB0(input_shape=(img_res, img_res, color), weights='imagenet', include_top= False)
   i=0
...
run_model_efficientnet(x_train, y_train, x_test, y_test, img_res, 20, validation= val_tuple, optim= optim, batch = batch , log_file_no='Eff_B0', color = color2, unfrozen = unfrz)
