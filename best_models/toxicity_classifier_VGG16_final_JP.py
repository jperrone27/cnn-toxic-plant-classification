...
from keras.applications import VGG16, MobileNet, EfficientNetB7, efficientnet, vgg16
...
def import_image_data(img_res=150, color = 'rgb'):
   ...
   for folder in meta_data['path']:
    ...
    #   image_arr_list.append(img_to_array(image))   
      #image_arr_list.append(vgg16.preprocess_input(img_to_array(image)))
...
def run_model_VGG16(X, Y, x_test, y_test, img_res, num_epochs, optim, validation = None, batch =None, log_file_no='', color = 'rgb', unfrozen = 2, dropout = 0.1):
   ...
   base_model = VGG16(input_shape=(img_res, img_res, color), weights='imagenet', include_top=False)
...
run_model_VGG16(x_train, y_train, x_test, y_test, img_res, 50, validation= val_tuple, optim= optim, batch= batch , log_file_no='VGG16_final', color = color2, unfrozen = unfrz, dropout = drop)
