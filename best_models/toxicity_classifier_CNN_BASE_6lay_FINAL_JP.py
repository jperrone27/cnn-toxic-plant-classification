...
from keras.applications import ResNet50, MobileNet, EfficientNetB7, efficientnet, resnet
...
def import_image_data(img_res=150, color = 'rgb'):
   ...
   for folder in meta_data['path']:
      ...
      image_arr_list.append((img_to_array(image) * 1.0/255))   
...
def run_model_base(X, Y, x_test, y_test, img_res, num_epochs, optim, validation = None, batch =None, log_file_no='', color = 'rgb', unfrozen = 2, dropout = 0.1):
   ...
   model = Sequential()
   model.add(Conv2D(filters = 32, kernel_size= (4,4), input_shape = (img_res,img_res, color), activation= 'relu'))
   ...
   visualkeras.layered_view(model, legend=True, font=font, color_map=color_map, to_file= str(log_file_no +'_keras diagram.png'))
...
def run_model_six_layer(X, Y, x_test, y_test, img_res, num_epochs, optim, validation = None, batch =None, log_file_no='', color = 'rgb', unfrozen = 2, dropout = 0.1):
...
# run_model_base(x_train, y_train, x_test, y_test, img_res, 50, validation= val_tuple, optim= optim, batch= batch , log_file_no='CNN_BASE_FINAL', color = color2, unfrozen = unfrz, dropout = drop)

run_model_six_layer(x_train, y_train, x_test, y_test, img_res, 50, validation= val_tuple, optim= optim, batch= batch , log_file_no='CNN_6layer_FINAL', color = color2, unfrozen = unfrz, dropout = drop)
