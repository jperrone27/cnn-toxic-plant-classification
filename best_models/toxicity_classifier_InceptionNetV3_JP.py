...
def run_model_InceptionV3(X, Y, x_test, y_test, img_res, num_epochs, optim, validation = None, batch =None, log_file_no='', color = 'rgb', unfrozen = 2, dropout = 0.1):
...
run_model_InceptionV3(x_train, y_train, x_test, y_test, img_res, 100, validation= val_tuple, optim= optim, batch = batch , log_file_no='InceptionV3_2', color = color2, unfrozen = unfrz, dropout = drop)
